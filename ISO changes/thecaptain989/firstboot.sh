...
LIB_VIDEOPATH=$(extract_answer "moviegnomes/library/videopath")
...
newzbin = ""
order = 0
dir = "$VIDEO_ROOT/#downloads/movies"
[[tv]]
priority = -100
pp = ""
name = tv
...
if [ "$INST_RADARR" = 1 ]; then
  ...
  [ "$i" = 10 ] && echo "WARNING: Default Radarr config files not created in $i seconds. Unconfigured container running?" || {
    ...
    sqlite3 $RADARR_ROOT/nzbdrone.db "UPDATE QualityDefinitions SET MinSize=11.8,MaxSize=127.2 WHERE Title='HDTV-1080p' OR Title='WEBDL-1080p';
      ...
      INSERT INTO Config(Key,Value) VALUES('recyclebin','$VIDEO_ROOT/#recycle/movies/');
      ...
      INSERT INTO RootFolders(Path) VALUES('$VIDEO_ROOT/movies/');
...
if [ "$INST_SONARR" = 1 ]; then
  ...
  [ "$i" = 10 ] && echo "WARNING: Default Sonarr config files not created in $i seconds. Unconfigured container running?" || {
    ...
    INSERT INTO Config(Key,Value) VALUES('cleanupmetadataimages','False');
    INSERT INTO Config(Key,Value) VALUES('recyclebin','$VIDEO_ROOT/#recycle/tv/');
    INSERT INTO Config(Key,Value) VALUES('autodownloadpropers','False');
    INSERT INTO RootFolders(Path) VALUES('$VIDEO_ROOT/tv/');
...
if [ "$INST_LIDARR" = 1 ]; then
  ...
  [ "$i" = 10 ] && echo "WARNING: Default Lidarr config files not created in $i seconds. Unconfigured container running?" || {
    ...
    INSERT INTO Config(Key,Value) VALUES('recyclebin','$MUSIC_ROOT/#recycle/');
    ...
    INSERT INTO RootFolders(Path) VALUES('$MUSIC_ROOT/');
...
<sources>
    ...
    <video>
        ...
        <source>
            <name>My Videos</name>
            <path pathversion="1">$LIB_VIDEOPATH</path>
...
<mediasources>
    <network>
        <location id="0">$LIB_VIDEOPATH</location>
        <location id="1">$LIB_MUSICPATH</location>
    </network>
</mediasources>
docker start kodi-headless
EOF
...
### Create cronjob to cleanup recycle bins and orphaned downloads
cat >/etc/cron.daily/mg-cleanup <<EOF
#!/bin/sh

# Daily cronjob to cleanup after Movie Gnomes.  Includes:
#  - Radarr and Sonarr Recycle Bin(s) (Lidarr takes care of itself)
...
echo "INFO: Created daily Movie Gnomes cleanup cronjob"
...
echo "INFO: Created Movie Gnomes webmin backup job"
